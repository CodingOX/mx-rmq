...
def main():
    ...
    if choice == "1":
        if upload_to_testpypi():
            print("\n✅ 已上传到TestPyPI!")
            print("🔗 访问: https://test.pypi.org/project/mx-rmq/")
            print("📦 测试安装: pip install -i https://test.pypi.org/simple/ mx-rmq")
    elif choice == "2":
        confirm = input("⚠️  确定要发布到正式PyPI吗? (y/N): ")
        if confirm.lower() == "y":
            if upload_to_pypi():
                print("\n🎉 已成功发布到PyPI!")
                print("🔗 访问: https://pypi.org/project/mx-rmq/")
                print("📦 安装: pip install mx-rmq")
    else:
        print("👋 退出发布流程")
